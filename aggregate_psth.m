...
for iprobe = 1:numel(probes)
    for f= 1:numel(RFCIspinfields)
        RFCIspinagg(iprobe).(RFCIspinfields{f}) = [];
    end
end
...
for iprobe = 1:numel(probes)
    for f= 1:numel(ori4paramsfields)
        ori4paramsagg(iprobe).(ori4paramsfields{f}) = [];
    end
end
...
%         end
        ...
        for f= 1:numel(RFCIspinfields)
            RFCIspinagg(iprobe).(RFCIspinfields{f}) = cat(1, RFCIspinagg(iprobe).(RFCIspinfields{f}), RFCIspin.(RFCIspinfields{f})(neuoi,:) );
        end
        ...
        for f= 1:numel(ori4paramsfields)
            ori4paramsagg(iprobe).(ori4paramsfields{f}) = cat(1, ori4paramsagg(iprobe).(ori4paramsfields{f}), ori4params.(ori4paramsfields{f})(neuoi,:) );
        end
